...
class ImgurDownloader:
    ...
    def __init__(
        ...
        if image_extension:
            self.album_title = self.main_key if file_name == "" else slugify(file_name)
        ...
        if not self.response or self.response.getcode() != 200:
            raise ImgurException(
                "[ImgurDownloader] URL:"+imgur_url+" HTTP Response Code %d" % response_code
            )
    ...
    def save_images(self, folder_name=""):
        ...
        for (counter, image) in enumerate(self.imageIDs, start=1):
            ...
            dl, skp = self.direct_download(
                image_url,
                os.path.join(os.path.dirname(path), slugify(os.path.basename(path))),
            )
        ...
        if os.path.isfile(path):
            ...
            try:
                ...
                if image_url != redirect_url:
                    self.log.debug(
                        "url, redirected_url = %s, %s" % (image_url, redirect_url)
                    )
                ...
                if self.delete_dne:
                    try:
                        ...
                        if self.debug:
                            print(
                                "[ImgurDownloader] Warning: DNE image not "
